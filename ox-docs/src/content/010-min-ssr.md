---
title: 01 Minimum SSR
description: Learn how to render HTML with Vue App created by createApp
---
...
SSR can be achieved with just `vue`, but Nuxt uses `vue-bundle-renderer/runtime` to create a SSR Renderer. So, let's create a renderer using `vue-bundle-renderer/runtime`.
...
Nitro uses `h3` as HTTP server, so let's create a h3 middleware that return HTML rendered by vue ssr renderer.
...
- How to create a Vue SSR renderer using `vue-bundle-renderer/runtime`
- How to create an h3 middleware that returns rendered HTML
- How to set up a basic HTTP server with h3

Next, we'll learn how to compile Vue Single File Components (SFC) on the server side.
