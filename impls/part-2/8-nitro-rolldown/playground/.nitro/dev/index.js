...
function use(app$1, arg1, arg2, arg3) {
	if (Array.isArray(arg1)) for (const i of arg1) use(app$1, i, arg2, arg3);
	else if (Array.isArray(arg2)) for (const i of arg2) use(app$1, arg1, i, arg3);
	else if (typeof arg1 === "string") app$1.stack.push(normalizeLayer({
		...arg3,
		route: arg1,
		handler: arg2
	}));
	else if (typeof arg1 === "function") app$1.stack.push(normalizeLayer({
		...arg2,
		handler: arg1
	}));
	else app$1.stack.push(normalizeLayer({ ...arg1 }));
	return app$1;
}
...
function toNodeListener(app$1) {
	const toNodeHandle = async function(req, res) {
		const event = createEvent(req, res);
		try {
			await app$1.handler(event);
		} catch (_error) {
			const error = createError(_error);
			if (!isError(_error)) error.unhandled = true;
			setResponseStatus(event, error.statusCode, error.statusMessage);
			if (app$1.options.onError) await app$1.options.onError(error, event);
			if (event.handled) return;
			if (error.unhandled || error.fatal) console.error("[h3]", error.fatal ? "[fatal]" : "[unhandled]", error);
			if (app$1.options.onBeforeResponse && !event._onBeforeResponseCalled) await app$1.options.onBeforeResponse(event, { body: error });
			await sendError(event, error, !!app$1.options.debug);
			if (app$1.options.onAfterResponse && !event._onAfterResponseCalled) await app$1.options.onAfterResponse(event, { body: error });
		}
	};
	return toNodeHandle;
}
...
//#endregion
//#region ../packages/nitro/dist/runtime/internal/render.mjs
...
var renderer_default = defineRenderHandler(async (event) => {
	...
	const rendered = await renderer2.renderToString({ url: req.url });
...
function htmlTemplate({ HEAD, APP }) {
	return `
  <!DOCTYPE html>
  <html>
  <head>
...
//#endregion
//#region ../packages/nitro/dist/presets/nitro/runtime/nitro-dev.mjs
