...
const distDir = process.env.APP_DIST_DIR!

let renderer: ReturnType<typeof createRenderer>
const getRenderer = async () => {
  if (renderer) return renderer
  const createApp = await import(join(distDir, '_entry.server.js')).then(
    m => m.default,
  )
...
function renderHTML({
  ...
  // distDir is packages/nuxt/dist/app, we need packages/nuxt/src/app
