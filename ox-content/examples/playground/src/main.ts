  ...
  const ast = JSON.stringify(
    {
      type: 'document',
      children: [
        { type: 'paragraph', note: 'Placeholder AST - NAPI bindings coming soon' }
      ]
    },
    null,
    2
...
function switchTab(activeTab: HTMLButtonElement, activeView: HTMLDivElement) {
  ...
  [previewTab, htmlTab, astTab].forEach(tab => tab.classList.remove('active'));
  [previewView, htmlView, astView].forEach(view => view.classList.remove('active'));
...
// Log version info
console.log('%c Ox Content Playground ', 'background: #e94560; color: white; font-weight: bold; padding: 4px 8px; border-radius: 4px;');
