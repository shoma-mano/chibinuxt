import { GlobbyOptions } from ".pnpm/globby@11.1.0/node_modules/globby";

type GenerateRoute = string | { route: string; payload: any };

type GenerateRoutesFunction = () => Promise<GenerateRoute[]> | GenerateRoute[];
type GenerateRoutesFunctionWithCallback = (
  callback: (err: Error, routes: GenerateRoute[]) => void
) => void;
...
export interface GenerateOptions {
  ...
  routes:
    ...
    | GenerateRoutesFunctionWithCallback;
  staticAssets: {
    base?: string;
    versionBase?: string;
    dir?: string;
