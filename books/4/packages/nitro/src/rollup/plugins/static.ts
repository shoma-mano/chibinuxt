import createEtag from ".pnpm/etag@1.8.1/node_modules/etag";
import {
  readFileSync,
  statSync,
} from ".pnpm/fs-extra@9.1.0/node_modules/fs-extra";
import mime from ".pnpm/mime@2.6.0/node_modules/mime";
...
export function staticAssets(context: NitroContext) {
  const assets: Record<
    string,
    { type: string; etag: string; mtime: string; path: string }
  > = {};

  const files = globby.sync("**/*.*", {
    cwd: context.output.publicDir,
    absolute: false,
  });
    ...
    renderChunk(code, chunk) {
      return {
        code:
          code +
          (chunk.isEntry
            ? 'global.mainDir="undefined"!=typeof __dirname?__dirname:require.main.filename;'
            : ""),
        map: null,
      };
    },
